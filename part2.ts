...
function addTask(task:string):number{ //retourne un nombre
...
function deleteTask(task:string):number{
    let index:number = myTasksArray.indexOf(task); //chercher ce aue c'est indexOf
    if (index>-1){
...
let numberOfTask:number=addTask("Eat Breakfeast"); //nombre de mot
